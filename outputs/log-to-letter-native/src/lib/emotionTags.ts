...
export const defaultRepresentativeEmotionTags: RepresentativeEmotionTags = {
  positive: ["positive:happy", "positive:enjoyable", "positive:hopeful", "positive:peaceful"],
  neutral: ["neutral:curious", "neutral:accepting", "neutral:instructive", "neutral:difficult"],
  negative: ["negative:irritated", "negative:tired", "negative:anxious", "negative:worried"]
};
...
export function normalizeRepresentativeEmotionTags(value: unknown): RepresentativeEmotionTags {
  ...
  return Object.fromEntries(emotionGroups.map((group) => {
    ...
    const filled = [...new Set([...valid, ...defaultRepresentativeEmotionTags[group]])].slice(0, 4);
    return [group, filled];
