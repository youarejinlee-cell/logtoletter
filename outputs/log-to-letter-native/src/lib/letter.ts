...
function getTone(entries: Entry[]) {
  ...
  if (average >= 70) return "에너지가 높게 기록됐어";
  if (average <= 40) return "에너지가 낮게 기록됐어";
  return "에너지가 오르내렸어";
}

function getEnergyRange(entries: Entry[]) {
  const high = [...entries].sort((a, b) => normalizeEnergyPercent(b.energy) - normalizeEnergyPercent(a.energy))[0];
  const low = [...entries].sort((a, b) => normalizeEnergyPercent(a.energy) - normalizeEnergyPercent(b.energy))[0];
  return {
    high: `${dateKey(high.createdAt)} · 에너지 ${normalizeEnergyPercent(high.energy)}%`,
    low: `${dateKey(low.createdAt)} · 에너지 ${normalizeEnergyPercent(low.energy)}%`
  };
}
...
export function buildWeeklyLetter(entries: Entry[], periodStart: Date, deliveredAt: Date, id = deliveredAt.toISOString()): Letter {
  ...
  if (!entries.length) {
    return {
      ...
      periodLabel,
      deliveredAt: `${dateKey(deliveredAt)}T00:00:00`,
      keyword: "기록되지 않은 한 주"
    };
  }
